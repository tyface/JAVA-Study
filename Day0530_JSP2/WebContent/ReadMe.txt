...
-servlet
	...
	다양한 요청을 한꺼번에 처리해야 함...
	web.xml :  프로젝트 전반적인 설정을 위해 xml형식으로 설정값 을 저장하는 파일
	서블릿이 처리할 요청 정의도 할 수 있음
...
요청목록
	...
	modifyForm : 수정화면 보여달라는 요청
				데이터베이스에서 현재 로그인된 사용자의 정보를 조회
				해당 정보를 수정 화면으로 전달
				수정화면에서 
				아이디,가입일 을 제외한 나머지 정보를 수정할 수 있게 
	 modify  : 수정로직처리 요청
	 			MemberSercie.modify()
...
-service 
	...
	modify(Member):boolean
		...
		성공하면true, 실패하면 false
	getAll() : List<Member> 데이터 베이스에서 모든 회원목록 가져오기
