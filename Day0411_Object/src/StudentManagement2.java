...
public class StudentManagement2 {
	...
	public StudentManagement2() { // 기본 생성자_정보를 기본값으로 초기화
	...
	public void showAllStudeunt() { // 모든 학생의 정보를 출력하는 메소드
		System.out.println("\r\n=============================================================================");
		Student[] arr = students.getStudents();
		for (int i = 0; i < students.size(); i++) {

			System.out.printf("이름 : %s / 학년 : %d / 평균 : %.2f / 국어 : %d / 영어 : %d / 수학 : %d \r\n", arr[i].getName(),
					arr[i].getGrade(), arr[i].getAverage(), arr[i].getScore().getKor(), arr[i].getScore().getEng(),
					arr[i].getScore().getMath());
		}
		System.out.println("=============================================================================\r\n");
	}
	...
	public void searchStudent() { // 학생의 이름을 입력받아 해당하는 학생을 출력하는 메소드
		...
		Student[] arr = students.getStudents();
		for (int i = 0; i < students.size(); i++) {
			if (arr[i].getName().equals(inputName)) {
				System.out.printf("이름 : %s / 학년 : %d / 평균 : %.2f / 국어 : %d / 영어 : %d / 수학 : %d \r\n", arr[i].getName(),
						arr[i].getGrade(), arr[i].getAverage(), arr[i].getScore().getKor(), arr[i].getScore().getEng(),
						arr[i].getScore().getMath());
			}
