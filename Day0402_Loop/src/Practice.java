...
public class Practice {
	public static void main(String[] args) {
		...
		int i = 0;		// 평균을 구할때 총합에서 나눌숫자(반복횟수)
		int sum = 0;	// 총합을 담을 변수
		...
		while (true) { // while start
			...
			while (!sc.hasNextInt()) {		// 오류검사_입력받은 값이 정수인지 확인
				sc.next();
			...
			if (num == RESULT_KEY) {			// -1 이라는 값을 받았을때 총합과 평균을 구한 후 출력
				if (i == 0) {					// 오류검사_총합과 평균을 구할 숫자가 없을때 재시작
					System.out.print("입력받은 숫자가 없습니다. ");
					continue;
				}
